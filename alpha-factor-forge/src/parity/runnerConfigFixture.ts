...
export async function buildRunnerConfigParityFixture(
  ...
  const configErrorSpecs: [string, JsonObject, string][] = [
    ...
    ['config-envelope-version-mismatch', broken((config) => { config.envelopeVersion = 'discovery-config-v4'; }), 'envelopeVersion must be one of discovery-config-v1, discovery-config-v2, discovery-config-v3'],
