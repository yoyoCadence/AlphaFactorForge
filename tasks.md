...
- [x] **RUNNER-CONFIG-001** — discovery run admission contract: strict config parsing, params-only candidate enumeration, and deterministic seeds (PR #66 Resolution slice 6).
  ...
  - Validation: fixture regeneration SHA-256 blob-identical (`c4845236…`); `npm run typecheck`; `npm test` (373, +34); `npm run build`; `cargo check --locked`; `cargo test --locked` (63, +16); targeted `rustfmt --check` clean on all new files; `npm run e2e` (25). Clippy was intentionally not run, matching the prior slices.
