...
- [ ] UI port — Slice 6-3: live signal readout at the current replay bar (entry/exit condition TRUE? + running position; reuses the signals services, no live fetch). Slice 6-2 (replay autoplay) done; then Slice 7 (library/export), and later Slice 8b (real Tauri window, if multi-monitor wanted). See the slice plan under In Progress.
...
- [ ] Port the legacy AlphaFactorForge PWA UI into the React/Tauri structure (incremental)
  ...
  - **Slice plan (small, one PR each):**
    ...
    - [ ] Slice 6: bar replay + live signals — step through candles bar-by-bar to watch the strategy trigger. Split into small PRs:
