...
- Next: UI port Slice 5d — chart buy/sell trade markers (entry ▲ / exit ▼ on `CandleChart` from `result.trades`). Slice 5b (parameter sweep: engine + heatmap UI + interactive cell-click apply / applied highlight across heatmap/form/chart) done; Slice 5a (holdout / out-of-sample comparison) done; strategy editor has params/blocks/code modes (code via the safe interpreter, manual-only). Then Slice 5c ("?" help markers) and Slice 6 (bar replay + live signals).
...
- [ ] UI port — Slice 5d: chart buy/sell trade markers (entry ▲ / exit ▼ on `CandleChart`). Slice 5b-3 (interactive sweep heatmap: cell-click apply + applied highlight across heatmap/form/chart) done; then Slice 5c ("?" help markers). See the slice plan under In Progress.
...
- [ ] Port the legacy AlphaFactorForge PWA UI into the React/Tauri structure (incremental)
  ...
  - **Slice plan (small, one PR each):**
    ...
    - [ ] Slice 5d (CURRENT): chart buy/sell trade markers — draw entry (buy ▲) / exit (sell ▼) markers on `CandleChart` from the latest backtest `result.trades` (positioned via the unit-tested `scale.ts`), like a normal trading terminal. Was deferred at Slice 3. Pass trades into the chart; toggle with the existing overlay row.
    - [ ] Slice 5c: clickable "?" help markers on major blocks/buttons — a reusable `HelpTip` (small `?` that toggles a short explanation popover) on 資料集 / 策略(+modes) / 執行模型 / Holdout / 回測績效 / 參數掃描 and key actions (執行回測 / 儲存結果 / 執行掃描 / 套用最佳). UI-only; no logic change.
