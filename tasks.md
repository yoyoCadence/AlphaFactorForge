...
- Next: reviewer Resolution for PERSIST-001 is recorded (handoff, PR #64) — D1 Option A rejected; implementation follows the revised Option C (migration `0002_validation_records`, immutable append-only records, atomic bundle save, shared metrics codec) after the handoff PR merges.
...
- [ ] PERSIST-001: persist the validation-run record — **Resolution recorded (revised Option C); implement after the handoff PR merges**.
  - The reviewer REJECTED the proposal's D1 Option A (`discovery_runs.config_json`) — that table stays the discovery-batch parent with input-only config. Implementation follows the Resolution in `handoffs/2026-07-19-persist-001-validation-record-proposal-v1.md`: migration `0002_validation_records.sql` (append-only, `gate_passed` CHECK **plus the addendum's D3 invariant CHECK — gate fail ⇒ score NULL, gate pass ⇒ score NOT NULL**, no update/delete), a self-contained immutable `validation-record-v1` snapshot (embargo derivation + split plan + Train/Validation metrics snapshots + full benchmark record incl. Random Entry distribution + full GateVerdict + ScoreBreakdown-or-null + contract versions), ONE atomic `save_validation_record` command (Train + Validation summaries/trades + record in one transaction, pre-validated), a shared `metricsCodec.ts` (extracted from reportExport), a Gate pass/fail discriminated union in the composer, and typed list/get read paths with mock parity.
  - Out of scope: UI wiring, lifecycle transitions, discovery runner, Test persistence.
