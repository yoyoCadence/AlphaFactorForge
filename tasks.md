...
- Post-PR #76 audit (2026-07-31): the executable runner is merged on `main` at `748ff91`, but the read-only project audit found correctness and audit-integrity gates that must land before RUNNER-UI-001. The ordered follow-ups and evidence are in `handoffs/2026-07-31-pr76-post-merge-audit-v1.md`. `BUG-RESULT-CONTEXT-001` (order #1) merged on 2026-08-01 as `38df26a` and is recorded under Done; `METRIC-002` (order #2) was expanded into an execution-ready spec on 2026-08-01 and is now in Next. Legacy-parity gaps remain split out as PARITY-001/002/003.
...
- [ ] **METRIC-002 (P1)** — base each UTC month's return on the previous month's closing equity (and the first month on `startEquity`) instead of dropping cross-month moves, shipped as a `metrics-v1` -> `metrics-v2` contract bump with TS/Rust parity and every dependent fixture regenerated. Expanded into the execution-ready format on 2026-08-01; spec: `docs/improvement-backlog.md` task `METRIC-002`. Scope deliberately excludes the persisted-record version gap — see `PERSIST-AUDIT-001` below, which the maintainer adjudicated on 2026-08-01 as a **required** successor, not an optional improvement.
...
- **METRIC-002 (P1)** — promoted to Next on 2026-08-01; see the Next section.
- [ ] **PERSIST-AUDIT-001 (P1)** — replace the Rust validation-record JSON shape check with strict version-dispatched DTO validation for the complete `validation-record-v1` and `bench-record-v1` evidence, including contract constants, hashes, split/embargo, metric snapshots, four deterministic benchmark identities, full Random Entry distribution, Gate, Score, and cross-row consistency. Reject unknown versions and mutation-test every required field before opening the transaction.
  - **Required successor to METRIC-002 (maintainer decision, 2026-08-01).** `METRIC-002` bumps the metric formula to `metrics-v2`, but `validation-record-v1` has no metrics contract field, so a record stored under the old monthly formula cannot be told apart from a new one. This task must add that metrics contract/version field and bump the record contract to `validation-record-v2` across the TS writer, the Rust writer, the Rust validator, and the record fixtures.
  - Every existing `validation-record-v1` row is legacy with an **unknown** formula version. It must never be defaulted to `metrics-v2`.
  - Until this lands, `RUNNER-UI-001` and any other flow that needs trustworthy persisted records stays blocked. Merging `METRIC-002` alone does not close the audit-integrity finding.
