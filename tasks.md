...
- [x] PERSIST-001 — immutable validation-record persistence (implements the PR #64 handoff Resolution, revised Option C + DB-invariant addendum).
  ...
  - Validation: `npm run typecheck`; `npm test` (298, +14); `npm run build`; `cargo check --locked`; `cargo test --locked` (20, +7); full `npm run e2e` (25) re-run after each mock-seam change.
