...
- Next: RS-CORE-003 (params signals + split/embargo parity) is implemented — RS-CORE-004 (deterministic benchmarks + mulberry32/Random Entry parity) unblocks when it merges; then RS-CORE-005, RUNNER-CONFIG/STORE/EXEC, and RUNNER-UI in order.
...
- [ ] Discovery-runner program — next slice RS-CORE-004 (deterministic benchmarks + mulberry32/Random Entry parity) once RS-CORE-003 merges, then strictly ordered: RS-CORE-005 → RUNNER-CONFIG-001 → RUNNER-STORE-001 → RUNNER-EXEC-001 → RUNNER-UI-001.
...
- [x] **RS-CORE-003** — pure Rust params-signals, validation-split, and embargo-derivation parity (PR #66 Resolution slice 3).
  - Added `src/parity/signalsSplitFixture.ts` + `npm run fixtures:signals-split` + committed `signals-split-parity-v1` envelope: 7 signal cases (hand-verified exact MA-cross index + one sample case per family, locking bar-0-never-signals and warm-up-NaN-false), 9 split cases (all five usable-bar residues, zero/non-zero embargo, the JS safe-integer extreme), 7 embargo cases (per-family lookbacks, holding allowance, unused-period usage-awareness), and 8 error cases HELD by the TS reference (generation + freshness execute the real TS functions and require the recorded fragment). Every leaf compares exactly — booleans and integers only; both languages assert the exact inventories by id.
  - Added pure Rust `discovery_core/signals.rs` (`params-signals-v1`), `split.rs` (`validation-split-v1`), and `embargo.rs` (`embargo-derivation-v1`); per D2 only params mode is ported — blocks/code and the expression interpreter stay TypeScript-only; unsupported ids fail closed with the identical message.
  - Validation: `npm run typecheck`; `npm test` (318, +3); `npm run build`; `cargo check --locked`; `cargo test --locked` (38, +4 parity suites passing first run); targeted `rustfmt --check` clean; fixture regeneration blob-identical; e2e untouched (no UI/mock surface).
