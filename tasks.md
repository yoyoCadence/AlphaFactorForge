...
- Next: RS-CORE-001 establishes the committed TS-reference parity harness foundation plus pure Rust candle/types and indicator parity. The resolved program then proceeds through RS-CORE-002..005, RUNNER-CONFIG/STORE/EXEC, and RUNNER-UI in order.
...
- [ ] **RS-CORE-001** — parity harness foundation plus pure Rust candle/types and indicator parity.
  - TS remains the reference and exports committed fixture candle inputs + expected indicator series; Rust consumes the same fixtures under the PR #66 tolerance/structure policy.
  - Sample generation stays TypeScript fixture tooling only and never becomes a Rust runtime data source. No DB, runner threads, events, or UI in this slice.
- [ ] Discovery-runner program — blocked behind RS-CORE-001, then strictly ordered: RS-CORE-002 → 003 → 004 → 005 → RUNNER-CONFIG-001 → RUNNER-STORE-001 → RUNNER-EXEC-001 → RUNNER-UI-001.
