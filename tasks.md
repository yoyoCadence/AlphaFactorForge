...
- [x] PERSIST-001 — immutable validation-record persistence (implements the PR #64 handoff Resolution, revised Option C + DB-invariant addendum).
  ...
  - Validation: `npm run typecheck`; `npm test` (293, +9); `npm run build`; `cargo check --locked`; `cargo test --locked` (19, +6); full `npm run e2e` (25) — the mock seam changed, so the whole browser suite was re-run locally.
