...
- Next discovery slice: RUNNER-EXEC-001; RUNNER-UI-001 remains blocked behind it.
...
- [x] **RUNNER-STORE-001** — discovery run/job store, migration 0003, and the atomic candidate commit (PR #66 Resolution slice 7).
  ...
  - Test discipline: the rollback claim is pinned by `a_failure_after_the_writes_rolls_everything_back`, whose failure fires AFTER the writes and which asserts the surviving rows keep the PREVIOUS commit's values. The weaker "assert nothing exists" shape passes whether or not the rollback works (its guard fires before any write) and is kept separately under a name saying so. Verified by mutation: flipping the transaction drop behaviour to `Commit` fails the real test and passes the weak one.
