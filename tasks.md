...
- Post-PR #76 audit (2026-07-31): the executable runner is merged on `main` at `748ff91`, but the read-only project audit found correctness and audit-integrity gates that must land before RUNNER-UI-001. The ordered follow-ups and evidence are in `handoffs/2026-07-31-pr76-post-merge-audit-v1.md`. `BUG-RESULT-CONTEXT-001` (order #1), `METRIC-002` (order #2), `PERSIST-AUDIT-001` (order #3), `RUNNER-OWNERSHIP-001` (order #4), `DATA-QUALITY-001` (order #5), and `BUG-SWEEP-CONTEXT-001` (order #6) are completed and recorded under Done. `STRATEGY-VALIDATION-001` is the next follow-up in the adjudicated order, and is the last correctness gate before RUNNER-UI-001. Legacy-parity gaps remain split out as PARITY-001/002/003.
...
- [x] **BUG-SWEEP-CONTEXT-001 (P1)** — bound every completed parameter sweep to the inputs it optimised over (2026-08-15, branch `fix/sweep-result-context`, branched from `origin/main` `a3fe2fe`).
