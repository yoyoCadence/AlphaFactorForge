...
- [ ] **RUNNER-UI-001b** — the discovery runner panel: start/pause/resume/cancel controls, throttled progress, a results list, recovered-run adoption on mount, the `?mock=1` discovery seam, and browser e2e. Built on the slice A boundary. **Must be expanded in `docs/improvement-backlog.md` to the execution-ready format before it starts** (slice A's entry there records the split and what B owns).
...
- [x] **RUNNER-UI-001a** — the typed, version-checked frontend boundary to the merged discovery runner (2026-08-16, branch `feat/runner-event-contract`, branched from `origin/main` `cbc3f42`).
