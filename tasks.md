...
- Next: RS-CORE-002 extends the committed TS-reference parity harness to the backtest engine, trades, equity, and METRIC-001 outputs. The resolved program then proceeds through RS-CORE-003..005, RUNNER-CONFIG/STORE/EXEC, and RUNNER-UI in order.
...
- [ ] **RS-CORE-002** — pure Rust backtest engine and metrics parity.
  - Extend the committed fixture envelope with reviewed trades/equity/metrics cases, including the execution contract and exact METRIC-001 non-finite statuses. Test remains unexecuted; no DB, runner threads, events, or UI.
- [ ] Discovery-runner program — blocked behind RS-CORE-002, then strictly ordered: RS-CORE-003 → 004 → 005 → RUNNER-CONFIG-001 → RUNNER-STORE-001 → RUNNER-EXEC-001 → RUNNER-UI-001.
...
- [x] **RS-CORE-001** — TypeScript-reference parity harness foundation plus pure Rust candle/types and indicator parity.
  ...
  - Fixture SHA-256 is stable across regeneration; 310 vitest + typecheck + build + rustfmt check + cargo check + 32 Rust tests + 25 Playwright e2e pass.
