...
- Next: UI port Slice 10 — chart pan/zoom, then optional Slice 8b (real Tauri second window). Slice 7 is complete: report JSON/CSV export and the SQLite-backed strategy library are wired into the React/Tauri app.
...
- [ ] UI port — Slice 10: chart pan/zoom (wheel zoom first, then drag-pan); afterward consider Slice 8b (real Tauri second window). See the slice plan under In Progress.
