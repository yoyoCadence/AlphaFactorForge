...
- [x] **RUNNER-UI-001a** — the typed, version-checked frontend boundary to the merged discovery runner (2026-08-16, branch `feat/runner-event-contract`, branched from `origin/main` `cbc3f42`).
  ...
  - Validation: `npm run typecheck`, `npm test` (806, +33), `npm run build`, `npm run e2e` (56, unchanged), `cargo check --locked`, `cargo test --locked` (152, +7).
...
- [x] **STRATEGY-VALIDATION-001 (P1)** — one runtime admission gate for a manual strategy's indicator parameters (2026-08-16, branch `fix/strategy-period-validation`, PR #99, merge `cbc3f42`).
  ...
  - **Cross-field rules are warnings, not errors** (adjudicated in the spec): `fastMA<slowMA`, `macdFast<macdSlow`, `rsiBuy<rsiSell` are computable-but-dubious hypotheses, the repo's own recorded judgment prunes rather than rejects them, and blocking them would blank the (20,20) cell of the default 2-D sweep. Tightening later is one line; loosening after the fact is not.
  - **Forced dependency inversion.** `discoveryConfig -> randomEntry -> backtestRunner` means a validator the runner imports cannot import `discoveryConfig`; the first attempt produced an ESM cycle that broke 26 existing tests with `discoveryConfig.contracts is missing key "gate"`. The domain table, `checkNumericParam`, `DISCOVERY_VALIDITY_RULE_IDS`, and `candidateValidity` therefore moved **verbatim** into the new module, and both discovery modules re-export them. Neutrality is mechanically proven: `discoveryConfig.test.ts`, `candidateEnumeration.test.ts`, and `runnerConfigFixture.test.ts` pass **unmodified**, and regenerating `benchmark-v1.json` / `runner-config-v1.json` changed **only** three `generator.sourceHashes` lines — every expected value byte-identical.
