...
- Next: PERSIST-001 (validation-run record persistence) is staged, blocked on reviewer decisions D1–D5 in `handoffs/2026-07-19-persist-001-validation-record-proposal-v1.md`; key verified finding: the per-segment Phase B columns already exist end to end (SQL + Rust DTO/UPSERT + TS interface) — only population and the run-level record's home are open.
...
- [ ] PERSIST-001: persist the validation-run record (audit trail for the completed Phase B pure layer) — **blocked on reviewer decisions**.
  - Proposal with decision points D1–D5 (run-record home: `discovery_runs.config_json` [recommended] vs summary-row envelope vs migration; `benchmark_result_json` shape; Phase B column semantics; JSON-safe run-record shape; scope/sequencing): `handoffs/2026-07-19-persist-001-validation-record-proposal-v1.md`.
  - Do not move to In Progress until the handoff's Resolution records the decisions; implementation follows the Resolution wherever it differs.
