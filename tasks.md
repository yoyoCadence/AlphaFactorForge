...
- Next: the discovery-runner PROGRAM proposal (RS-CORE-001..005 parity ports + RUNNER-001/002) is staged, blocked on reviewer decisions D1–D7 in `handoffs/2026-07-19-runner-001-design-proposal-v1.md`; key fork: D1 computation location (recommended: the v3-mandated phased Rust engine port with cross-language parity fixtures).
...
- [ ] Discovery-runner program (RS-CORE-001..005 + RUNNER-001/002) — **blocked on reviewer decisions**.
  - Mode A proposal with decision points D1–D7 (computation location, v1 candidate space, cross-language parity harness, run-config schema, state machine/checkpoint/events, promotion policy, slice order): `handoffs/2026-07-19-runner-001-design-proposal-v1.md`.
  - Architectural ground truth already recorded: `STRATEGY_DISCOVERY.md` §4 v3 定案 (backend thread-pool backtests, `discovery://` event protocol, SQLite checkpoint) and the SCORE-001/PERSIST-001 Resolutions (runner owns promotion policy, run→jobs state machine, and the future `discovery_run_id` migration).
  - Do not move any slice to In Progress until the handoff's Resolution records the decisions.
