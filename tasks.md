...
- Post-PR #76 audit (2026-07-31): the executable runner is merged on `main` at `748ff91`, but the read-only project audit found correctness and audit-integrity gates that must land before RUNNER-UI-001. The ordered follow-ups and evidence are in `handoffs/2026-07-31-pr76-post-merge-audit-v1.md`. `BUG-RESULT-CONTEXT-001` (order #1) merged on 2026-08-01 as `38df26a`; `METRIC-002` (order #2) completed locally on 2026-08-07 and is recorded under Done. `PERSIST-AUDIT-001` is now Next as the required audit-version successor. Legacy-parity gaps remain split out as PARITY-001/002/003.
