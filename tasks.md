...
- Post-PR #76 audit (2026-07-31): the executable runner is merged on `main` at `748ff91`, but the read-only project audit found correctness and audit-integrity gates that must land before RUNNER-UI-001. The ordered follow-ups and evidence are in `handoffs/2026-07-31-pr76-post-merge-audit-v1.md`. `BUG-RESULT-CONTEXT-001` (order #1) is complete and recorded under Done; `METRIC-002` is next in that order but needs a Planner spec expansion before it may be promoted to Next. Legacy-parity gaps remain split out as PARITY-001/002/003.
...
- None. `METRIC-002 (P1)` is the adjudicated next task in the handoff order, but it stays in Backlog until a Planner expands it into the execution-ready Files / Exact plan / Non-goals / Validation / Acceptance format (`docs/improvement-backlog.md` currently expands `BUG-RESULT-CONTEXT-001` only, and its 2026-07-31 addendum requires that expansion before promotion).
...
- [x] **BUG-RESULT-CONTEXT-001 (P1)** — bind every completed interactive backtest to an immutable execution snapshot (2026-07-31, `fix/backtest-result-context` @ `8a8015f`, branched from `origin/main` `0ebe6a8`; PR #78 open as draft for review).
  ...
  - Scope note: the DEV-only `mockClient` gained a `?mock=1&candleDelay=<ms>` latency knob — the one file outside the spec's "Files likely affected" list, authorized by the maintainer on 2026-07-31 because the spec's deterministic late-load regression cannot exist without it. It sits behind the existing `import.meta.env.DEV` guard and is absent from production builds. Sweep-result provenance (`BUG-SWEEP-CONTEXT-001`), metric formulas (`METRIC-002`), Rust, schema, and the report schema were not touched.
  - Validation: `npm run typecheck`, `npm test` (409, +27), `npm run build`, and `npm run e2e` (28, +3) pass. No Rust files changed, so `cargo` was not re-run. Manual check: sample -> run -> edit fee -> metrics/Save/Export disappear behind the 已失效 notice -> re-run -> all return with matching metadata.
