...
- Next discovery slice: RUNNER-EXEC-001 is In Progress (unblocked by the PR #74 merge); RUNNER-UI-001 remains blocked behind it. Legacy-parity gaps split out as PARITY-001/002/003.
...
- [ ] **RUNNER-EXEC-001** — fixed CPU worker pool, Tauri commands, pause/resume/cancel, single writer, and versioned events; no frontend UI.
  - Unblocked 2026-07-30 by the RUNNER-STORE-001 merge (PR #74, `114c55a`). RUNNER-UI-001 stays blocked behind it. Rust backend computation is final; hidden WebView and Node sidecar are rejected. Cross-run result reuse remains Backlog until a versioned immutable execution cache exists.
  - Acceptance conditions carried from the 2026-07-30 handoff:
    - REMOVE the module-level `#![allow(dead_code)]` in `src-tauri/src/db/discovery.rs` when the commands wire the store, and fix whatever dead code then surfaces.
    - `discovery_core/metrics.rs`'s UTC timestamp `.expect()` must not remain on any path a runner input can reach. The recorded assumption that RUNNER-CONFIG guarantees chrono-representable timestamps does NOT hold — config validates the run envelope, while these timestamps come from dataset candles — so EXEC must validate at the execution boundary or make the core return a propagated error. Add a regression with an out-of-range timestamp proving no panic, a correct failure state, and a persisted `error_message`.
    - Emit completion events only AFTER the database transaction commits.
    - A late worker result arriving after cancel/fail must neither write candidate output nor emit a success event.
    - Startup orphan recovery only pauses orphaned `running` runs and requeues in-flight jobs; it never resumes CPU work automatically.
...
- [x] **RUNNER-STORE-001** — discovery run/job store, migration 0003, and the atomic candidate commit (PR #66 Resolution slice 7).
  ...
  - Documented in `docs/discovery-runner-store-contract.md`. `db/discovery.rs` carries a module-level `#![allow(dead_code)]` because nothing in a non-test build calls the store yet; RUNNER-EXEC-001 must remove it when it wires the commands.
