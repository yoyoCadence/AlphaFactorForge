...
- Next: RS-CORE-002 (Rust backtest engine + metrics parity) is implemented — RS-CORE-003 (params signals + split/embargo parity) unblocks when it merges; the resolved program then proceeds through RS-CORE-004..005, RUNNER-CONFIG/STORE/EXEC, and RUNNER-UI in order.
...
- [ ] Discovery-runner program — next slice RS-CORE-003 (params signals + split/embargo parity) once RS-CORE-002 merges, then strictly ordered: RS-CORE-004 → 005 → RUNNER-CONFIG-001 → RUNNER-STORE-001 → RUNNER-EXEC-001 → RUNNER-UI-001.
...
- [x] **RS-CORE-002** — pure Rust backtest engine and metrics parity (PR #66 Resolution slice 2).
  ...
  - Validation: `npm run typecheck`; `npm test` (313, +2); `npm run build`; `cargo check --locked`; `cargo test --locked` (34, +2 parity suites); e2e untouched (no UI/mock surface).
