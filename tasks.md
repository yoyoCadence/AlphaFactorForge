...
- Next: UI port Slice 5c — clickable "?" help markers (reusable `HelpTip`) on major blocks/buttons. Slice 5d (chart buy/sell trade markers ▲/▼ from `result.trades`) done; Slice 5b (parameter sweep + interactive heatmap + applied highlight) done; Slice 5a (holdout) done; strategy editor has params/blocks/code modes (code via the safe interpreter, manual-only). Then Slice 6 (bar replay + live signals).
...
- [ ] UI port — Slice 5c: clickable "?" help markers on major blocks/buttons (reusable `HelpTip`). Slice 5d (chart buy/sell trade markers) done. See the slice plan under In Progress.
