...
- [ ] UI port — Slice 7-2: export UI + file write (匯出 JSON/CSV buttons calling the 7-1 formatters). Slice 7-1 (pure export formatters) done; then 7-3 (strategy library), and backlog Slice 10 (chart pan/zoom) + Slice 8b (real Tauri window). See the slice plan under In Progress.
...
- [ ] Port the legacy AlphaFactorForge PWA UI into the React/Tauri structure (incremental)
  ...
  - **Slice plan (small, one PR each):**
    ...
    - [ ] Slice 7: strategy library + report (JSON/CSV) export. Split into small PRs:
      ...
      - [ ] Slice 7-2: export UI + file write — 「匯出 JSON / 匯出 CSV」 buttons on 回測績效 that call the 7-1 formatters. Decide the write path: Tauri `save_report` command + save dialog (the intended local-first route, touches `src-tauri`) vs a browser `Blob`+anchor download for dev/mock. Keep the typed `tauri-client` wrapper; dev/mock can no-op or download.
      - [ ] Slice 7-3: strategy library — list saved strategies (needs a `list_strategies` Tauri command + typed client) and load one back into the form; optional delete. Replaces the prototype's localStorage `cd_stratlib`.
    - [ ] Slice 10 (user-requested 2026-07-01; deferred pan/zoom, low priority): chart pan/zoom in the ported app (the legacy prototype had it; the port has been static fit-to-width since Slice 3). Introduce a visible-window state and reconcile it with replay `upto` + `maxBars`. Split: 10-1 wheel zoom (anchor on the bar under the cursor; a 重置 back to fit) · 10-2 drag-pan (mousedown+drag; distinguish from the Slice 9 hover crosshair — press = pan, release = hover). Touches `CandleChart` heavily; do after Slice 7.
