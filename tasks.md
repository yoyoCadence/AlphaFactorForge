...
- [ ] **RUNNER-UI-001** — typed frontend wrappers plus throttled progress/results UI. **Unblocked as of 2026-08-16**: every gate it waited on (BUG-RESULT-CONTEXT-001, METRIC-002, PERSIST-AUDIT-001, RUNNER-OWNERSHIP-001, DATA-QUALITY-001, BUG-SWEEP-CONTEXT-001, STRATEGY-VALIDATION-001) is in Done. A Planner must expand it in `docs/improvement-backlog.md` before it is promoted. When promoted, replace the stale frontend event DTOs with the backend `discovery-event-v1` contract, add the active-run wrapper, and make the throttle cancelable and stale-timer-safe.
...
- [x] **STRATEGY-VALIDATION-001 (P1)** — one runtime admission gate for a manual strategy's indicator parameters (2026-08-16, branch `fix/strategy-period-validation`, branched from `origin/main` `be4e3c6`).
