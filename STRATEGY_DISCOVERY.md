...
- **市場資料取得（P07）**：`alpha-factor-forge-service fetch` 以工作區 owner 身分做一次有界取得。單位計畫為「月封存 →（該月未公布時）日封存 →（仍未公布且允許時）同交易所 REST 的已收盤 bar」；每個封存檔以**官方公布的 SHA-256** 校驗、只讀單一 ZIP entry（檢查 entry 數、長度與 CRC-32）、原件存入 artifact store 並由 `market_provenance` 參照，**被拒絕的取得連同失敗的 bytes 一起保留**。時間單位以**整檔**判定（封存自 2025-01 由毫秒改微秒，已實測確認），同檔混用或無法精確換算即拒，判定結果寫入該檔的 `requestScope`。合併時同一根 bar 內容不一致即拒（`source_conflict`），尚未收盤的 bar 不匯入。匯入走既有 `import_dataset_with_candles`（識別與逐根閘門不變），最後交給 P06 `build_snapshot`。**兩種涵蓋率不可混淆**：snapshot 回答「資料集自身是否完整」，`IngestReport.rangeCoverage` 回答「要求的區間是否拿到」——後者缺漏會以 `scope: "requested-range"` 的事件拒絕整次取得。同一交易所的封存與 REST 視為同一來源（`sourcesShareOrigin`），跨交易所永不合併。詳見 [`docs/market-source-binance-v1.md`](docs/market-source-binance-v1.md)。
