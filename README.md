...
- 2026-07-16 的 [npm audit 盤點](docs/security-audit-npm.md) 為 5 個 dev-only 受影響套件節點（production audit 為 0）；修復需協調 Vite/Vitest major 升級，**不要直接跑 `npm audit fix --force`**。
...
- The [2026-07-16 npm audit triage](docs/security-audit-npm.md) found five affected dev-only package nodes and zero production findings; remediation requires a coordinated Vite/Vitest major upgrade. **Do not run `npm audit fix --force`.**
...
- [2026-07-16 の npm audit 調査](docs/security-audit-npm.md)では dev-only の影響パッケージノード 5 件、production 0 件を確認しました。修正には Vite/Vitest の major 更新をまとめて検証する必要があり、**`npm audit fix --force` は実行しないでください**。
