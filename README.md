...
>
> **P09（2026-09-21）已實作 Tiingo US ETF adapter**：`fetch-tiingo` 逐檔下載與稽核 SPY／QQQ／VTI／TLT／GLD，保存原始價、調整價及公司事件原件，核對付款日並回報資格阻擋。憑證由 Rust 讀取 Windows 認證管理員；[設定與使用步驟](docs/market-source-tiingo-v1.md)。自動測試可用；真實帳戶下載驗收仍待 token 配置，不能宣稱資料已齊全。TW ETF、排程及 paper 尚未交付。
