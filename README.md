...
| DSL 安全驗證與執行 | `strategy-dsl-v1` 以 TS／Rust 白名單 validator 與 evaluator 拒絕不合法結構；固定 DSL 可經 `discovery-config-v2` 進入 backend runner | **P11 已完成；AI 生成／approve 尚待 P15** |
...
> 現況界線：目前可直接展示與驗證的是歷史回測工作區、可執行 `strategy-dsl-v1`（含 backend runner admission）、Discovery backend 與下列檢驗工具；完整 AI 生成／人工 approve 操作流程尚未在前端接線。本 README 會同時說明產品目標與目前可查驗的能力，不把 roadmap 當成已完成介面。

> **P08（2026-09-20）已完成 ETF 日線純核心契約**：股息應收／付款、分割、原幣成本與實際時間年化，並有 TS／Rust 共用驗證案例。開發介面與限制見 [ETF 語意說明](docs/etf-semantics-v1.md)。目前 UI 回測沿用既有契約；成交／paper 接線仍待 P18+。
>
> **P09（2026-09-21）已實作 Tiingo US ETF adapter**：`fetch-tiingo` 逐檔下載與稽核 SPY／QQQ／VTI／TLT／GLD，保存原始價、調整價及公司事件原件，核對付款日並回報資格阻擋。憑證由 Rust 讀取 Windows 認證管理員；[設定與使用步驟](docs/market-source-tiingo-v1.md)。自動測試可用；真實帳戶下載驗收仍待 token 配置，不能宣稱資料已齊全。
>
> **P10（2026-09-21）已完成 FinMind／TWSE 台灣 ETF adapter**：`fetch-tw-etf` 逐檔處理 0050／006208／0056／00878／00713，以 FinMind raw 日線建立資料集、以 TWSE 月行情逐列核對，保存 2025 交易日、停牌、配息付款與分割證據。[設定、使用方式與真實來源結果](docs/market-source-tw-etf-v1.md)。五檔小範圍 smoke 均通過資料稽核；範例成本未確認，所以 snapshot 刻意維持 degraded。排程及 paper 尚未交付。
