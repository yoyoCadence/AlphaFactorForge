...
> Status: parity record plus maintainer decisions. TEST-002 created the observation-only baseline; the maintainer adopted `backtest-execution-contract.md` on 2026-07-14. BUG-002 through BUG-004 now implement the accounting, fill-policy, direction, and input-boundary decisions with intentional golden updates.
...
- **Current engine (BUG-003):** A signal creates a pending order only when another tested candle exists (`index.ts:219-230`). The order fills at the start of the next candle using that candle's open, index, and timestamp before risk checks or mark-to-market equity (`:167-182`). A final-bar signal therefore cannot create an out-of-range fill.
...
- **Current engine (BUG-003):** Risk exits choose a gap-aware base from the candle open and threshold, then apply normal closing-side slippage (`index.ts:184-197`). Long exits use sell-side slippage and short exits use buy-side slippage.
...
- **Current engine (BUG-003):** The `if ... else if` ordering chooses SL before TP when both thresholds are touched (`index.ts:190-196`).
...
- **Current engine (BUG-002):** Entry budget explicitly includes notional plus its fee (`index.ts:150-160`). A short reserves 1× entry notional as collateral; mark-to-market equity is free cash + collateral + unrealized PnL (`:234-242`). Closing releases collateral and emits fee-inclusive PnL/PnL% (`:123-147`).
...
- **Current engine (BUG-002 + BUG-003):** A remaining position is force-closed at the final candle close using normal closing-side slippage (`index.ts:245-248`). The final equity point is replaced with settled cash including fees (`:249-251`), and metrics use configured `startEquity` plus that settled endpoint (`:253-259`; `metrics/index.ts:86-110`).
...
- **Current engine (BUG-004):** `requestedBothSide()` maps entry to long and exit to short, with entry precedence when both are true (`index.ts:163-165`). Close fills reverse immediately when the requested side differs (`:199-208`); `nextOpen` schedules the same close-then-open transition for the following tested candle (`:219-225`, executed at `:172-181`).
...
- **Decision:** Restore legacy reversal semantics for the current two-signal model. Entry requests long, exit requests short, an already-held requested side is retained, and entry wins a simultaneous entry/exit bar. A future explicit four-signal model remains a separately versioned change.
...
- **Current engine (BUG-004):** `runBacktest()` validates normalized config fractions before processing data (`index.ts:84-108`). Sizing/fees/slippage accept `[0, 1]`; active SL/TP accepts `(0, 1]`; invalid values throw instead of being clamped. The product runner remains responsible for percentage conversion, the zero-size legacy fallback, the 1% size floor, and negative fee/slippage clamps (`backtestRunner.ts:38-54`).
- **Legacy:** The same percentage-unit fallback and clamps live inside `runBacktestCore()` (`AlphaFactorForge.dc.html:1065-1067`).
- **Decision:** Keep percentage conversion and legacy fallbacks exclusively in the product runner; make the pure core fail fast on invalid normalized fractions. Direct core/worker/DSL callers must pass the normalized contract and cannot receive silent rebates or oversized positions.
...
The maintainer adopted the target semantics in `backtest-execution-contract.md` on 2026-07-14. BUG-002 resolves the accounting and settled-equity portions in rows 4–5; BUG-003 resolves rows 1–3 plus EOD slippage; BUG-004 resolves rows 6–7. Golden values preserve each implemented contract and are updated only when an approved correction intentionally changes output.
