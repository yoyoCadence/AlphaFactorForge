...
- **Test coverage:** Vitest tests cover indicators, Strategy DSL validation, and backtest determinism. Latest local baseline in `alpha-factor-forge/`: `npm install` completed, `npm test` passed 25 tests across 3 files, `npm run typecheck` passed after narrow strict-mode fixes, and `npm run build` passed.
- **Deployment / cache notes:** Target app identifier is `com.alphafactorforge.desktop`. SQLite is created in the OS app-data directory on first Tauri startup. Tauri build/dev requires generated icon files under `alpha-factor-forge/src-tauri/icons/*`; the scaffold currently has an icon README placeholder. Rust/Cargo were not available on PATH during the latest check, so native Tauri verification is still pending. The folder is not currently a valid Git repository even though a `.git` directory exists; `git status` fails.
